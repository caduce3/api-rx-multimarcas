...
export class PegarProdutosUseCase {
    ...
    async execute({
        page, nome, descricao, preco, quantidadeDisponivel
    }: pegarProdutosUseCaseRequest): Promise<pegarProdutosUseCaseResponse> {

        if (page <= 0) page = 1;

        const take = 10;
        const { produtos, totalCount } = await this.produtoRepository.pegarProdutos(take, page, nome, descricao, preco, quantidadeDisponivel);
