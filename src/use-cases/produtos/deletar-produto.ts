...
export class DeletarProdutoUseCase {
    ...
    async execute({ id_produto }: DeletarProdutoRequest): Promise<DeletarProdutoResponse> {

        const produtoExiste = await this.produtosRepository.findProdutoById(id_produto);
        if(!produtoExiste) throw new ProdutoNaoExiste()
        
        const deletarProduto = await this.produtosRepository.deletarProduto(id_produto)
